"""
Three-Regime Wave Equation Experiment
======================================

Wave equation: ∂²u/∂t² = c²∇²u

Tests the three-regime methodology on a hyperbolic PDE representing
...
class WavePhysicsInformedNN(nn.Module):
    """
    Physics-Informed Neural Network for 1D Wave equation:
    ∂²u/∂t² = c²∂²u/∂x²
    
    where c = 1.0 is wave speed
    """
...
def wave_loss(model, x, t, c=1.0):
    """
    Physics-informed loss for wave equation
    ...
    # Initial velocity
    u_t_init = torch.autograd.grad(u_init.sum(), t_zero, create_graph=True)[0]
    
    # Combined loss
    loss_pde = torch.mean(pde_residual ** 2)
    loss_bc = torch.mean(u_left ** 2) + torch.mean(u_right ** 2)
    loss_ic_pos = torch.mean((u_init - u_init_exact) ** 2)
    loss_ic_vel = torch.mean(u_t_init ** 2)
    
    total_loss = loss_pde + 10.0 * loss_bc + 10.0 * loss_ic_pos + 10.0 * loss_ic_vel
    
    return total_loss, {
        'pde': loss_pde.item(),
        'boundary': loss_bc.item(),
        'initial_pos': loss_ic_pos.item(),
        'initial_vel': loss_ic_vel.item()
    }
...
def evaluate_accuracy(model, n_test=1000):
    ...
    with torch.no_grad():
        ...
        # Analytical solution: u(x,t) = sin(πx)cos(πct) for c=1
        u_exact = torch.sin(np.pi * x_test) * torch.cos(np.pi * t_test)
...
def run_three_regime_wave(epochs=3000, save_dir='chapter4/results/three_regime_wave', seed=42):
    """Run three-regime training on Wave equation"""
    
    print("="*80)
    print("THREE-REGIME WAVE EQUATION EXPERIMENT")
    print("="*80)
    ...
    config = {
        'epochs': epochs,
        'lr': 1e-2,  # FIXED: Optimal learning rate found via hyperparameter search
        'regularization': 1e-4,
    ...
    model_continuous = WavePhysicsInformedNN(config['hidden_sizes']).to(device)
    ...
    model_passive = WavePhysicsInformedNN(config['hidden_sizes']).to(device)
    ...
    model_active = WavePhysicsInformedNN(config['hidden_sizes']).to(device)
    ...
    def compute_loss_active(reg_weight):
        loss, _ = wave_loss(model_active, data['x_domain'], data['t_domain'])
...
if __name__ == '__main__':
    results = run_three_regime_wave()
    print("\n" + "="*80)
    print("WAVE EQUATION EXPERIMENT COMPLETE")
    print("="*80)