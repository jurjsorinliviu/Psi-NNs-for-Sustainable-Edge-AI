"""
Three-Regime Heat Equation Experiment
======================================

Heat (diffusion) equation: ∂u/∂t = α∇²u

Tests the three-regime methodology on a parabolic PDE representing
...
class HeatPhysicsInformedNN(nn.Module):
    """
    Physics-Informed Neural Network for 1D Heat equation:
    ∂u/∂t = α∂²u/∂x²
    
    where α = 0.01 is thermal diffusivity
    """
...
def heat_loss(model, x, t, alpha=0.01):
    """
...
def evaluate_accuracy(model, n_test=1000):
    ...
    with torch.no_grad():
        ...
        t_test = torch.rand(n_test, 1).to(device) * 0.5
        
        u_pred = model(x_test, t_test)
        u_exact = torch.sin(np.pi * x_test) * torch.exp(-np.pi**2 * 0.01 * t_test)
...
def run_three_regime_heat(epochs=3000, save_dir='chapter4/results/three_regime_heat', seed=42):
    """Run three-regime training on Heat equation"""
    
    print("="*80)
    print("THREE-REGIME HEAT EQUATION EXPERIMENT")
    print("="*80)
    ...
    model_continuous = HeatPhysicsInformedNN(config['hidden_sizes']).to(device)
    ...
    model_passive = HeatPhysicsInformedNN(config['hidden_sizes']).to(device)
    ...
    model_active = HeatPhysicsInformedNN(config['hidden_sizes']).to(device)
    ...
    def compute_loss_active(reg_weight):
        loss, _ = heat_loss(model_active, data['x_domain'], data['t_domain'])
...
if __name__ == '__main__':
    results = run_three_regime_heat()
    print("\n" + "="*80)
    print("HEAT EQUATION EXPERIMENT COMPLETE")
    print("="*80)